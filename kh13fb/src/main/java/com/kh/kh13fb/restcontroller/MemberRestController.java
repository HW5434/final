...
public class MemberRestController {
	...
	public ResponseEntity<MemberLoginVO> login(@RequestBody MemberDto memberDto) {
		...
		if (isValid) {// 성공- MemberLoginVO(200)
			...
			return ResponseEntity.ok()
					.body(MemberLoginVO.builder().memberNo(findDto.getMemberNo()).memberId(findDto.getMemberId())
							.memberGrade(findDto.getMemberGrade()).accessToken(accessToken).refreshToken(refreshToken)
							.build());// 200
		...
		try {
			...
			return ResponseEntity.ok()
					.body(MemberLoginVO.builder().memberId(memberDto.getMemberId())
							.memberGrade(memberDto.getMemberGrade()).accessToken(accessToken)
	...
	public ResponseEntity<MemberDto> findId(@RequestBody MemberDto memberDto) {
		...
		boolean isValid = findIdMemberDto != null
				&& findIdMemberDto.getMemberEmail().equals(memberDto.getMemberEmail());
		if (isValid) {
